...
internal val MIGRATION_9_10 = object : Migration(9, 10) {
    override fun migrate(connection: SQLiteConnection) {
        connection.execSQL("ALTER TABLE `reading_progress` ADD COLUMN `dirty` INTEGER NOT NULL DEFAULT 0")
    }
}

internal val MIGRATION_10_11 = object : Migration(10, 11) {
    override fun migrate(connection: SQLiteConnection) {
        connection.execSQL("ALTER TABLE `downloads` ADD COLUMN `durationMs` INTEGER DEFAULT NULL")
    }
}

/** issue #257 — series name/number snapshot for Home's Continue shelves. */
internal val MIGRATION_11_12 = object : Migration(11, 12) {
    override fun migrate(connection: SQLiteConnection) {
        connection.execSQL("ALTER TABLE `reading_progress` ADD COLUMN `series` TEXT DEFAULT NULL")
        connection.execSQL("ALTER TABLE `reading_progress` ADD COLUMN `seriesIndex` REAL DEFAULT NULL")
    }
}
