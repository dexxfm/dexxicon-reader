...
/** Result of the browser auth step: the code + the verifier needed for server exchange. */
sealed interface OidcAuthResult {
    data class Code(val code: String, val codeVerifier: String) : OidcAuthResult
...
class OidcAuthFlow(context: Context) {
    ...
    fun parseResult(data: Intent?): OidcAuthResult {
        if (data == null) return OidcAuthResult.Cancelled
